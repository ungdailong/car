...
class xemoimodel extends CI_Model {
	...
	function countCarsByCat(){
		$query = $this->db->query("select count(*) as countt,category_id from tbl_cars where hide = 'N' GROUP BY category_id");
		return $query -> result_array();
	}
	...
	function getXeByCategoryId($category_id,$offset,$pagesize){
		if(is_array($category_id)){
			$category_id_str = implode(',', $category_id);
			$query = $this->db->query("select * from tbl_cars where hide = 'N' and category_id IN (".$category_id_str . ") ORDER BY id DESC LIMIT $offset,$pagesize");

		}
		else
			$query = $this->db->query("select * from tbl_cars where hide = 'N' and category_id =".$category_id . " ORDER BY id DESC LIMIT $offset,$pagesize");
		return $query -> result_array();
	}
}